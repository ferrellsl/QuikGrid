...
Users who wish to create VRML models should note that QuikGrid generates VRML version 1.0 files which are ancient and many more modern applications that purport to read/write VRML files no longer support VRML 1.0.  To address this issue, I've added a converter/updgrade tool in the VRML 1.0 to VRML 2.0 Converter folder.  I've also added a VRML Viewer app in the VRML Viewer folder.  The viewer is quite old and is based on the Coin3D library.  It will read and view VRML 1.0 and 2.0 files.  If it crashes on your system, run it in Windows 98/ME compatibility mode.  Loading VRML 1.0 files seems to take ages but version 2.0 files load quickly.  A sample VRML file of Wizard Island is located in the Sample Data folder.
