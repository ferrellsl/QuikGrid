...
This latest version now supports exporting (Structured Vector Graphics) SVG files instead of exporting Windows Meta Files (WMF) to the clipboard.  Exporting WMF files made sense in QuikGrid's early days when data sets were small and sparse, but LIDAR files and current survey data files have become enormous.  Windows 11 also limits the size of WMF clipboard files to 4MB and QuikGrid can easily exceed this limit even when the smallest of LIDAR files are loaded. If you absolutely must use WMF exports, then please use an earlier version of QuikGrid but bear in mind that you will eventually hit the WMF size limit imposed by Windows.  You can still export BMP files to the clipboard in this version and earlier versions as well, but SVG images are preferred because they are true vector files and can be scaled to any size without pixelation and Windows won't limit your SVG file sizes.
